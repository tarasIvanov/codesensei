...
def test_empty_findings_yields_no_comments_and_verdict_body() -> None:
    payload = build_payload(_result([], verdict=Verdict.APPROVE), "APPROVE")
    assert payload["comments"] == []
    assert payload["event"] == "APPROVE"
    body = payload["body"]
    assert isinstance(body, str)
    assert body.startswith("Verdict: APPROVE")
    assert "Generated by CodeSensei via openai" in body
