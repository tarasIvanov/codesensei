...
def test_db_down_returns_503_and_flags_db_and_vector():
    status, body = build_envelope(
        {"db": "down", "vector": "unknown"}, "ok", *_ok_providers()
    )
    assert status == 503
...
def test_redis_down_returns_503_and_flags_only_redis():
    status, body = build_envelope({"db": "ok", "vector": "ok"}, "down", *_ok_providers())
    assert status == 503
...
def test_vector_missing_returns_503_and_flags_only_vector():
    status, body = build_envelope({"db": "ok", "vector": "missing"}, "ok", *_ok_providers())
    assert status == 503
    assert body["db"] == "ok"
    assert body["redis"] == "ok"
...
def test_db_and_redis_down_lists_all_failing():
    status, body = build_envelope(
        {"db": "down", "vector": "unknown"}, "down", *_ok_providers()
    )
    assert status == 503
