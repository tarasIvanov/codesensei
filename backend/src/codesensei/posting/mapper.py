...
from codesensei.review.schema import Finding, ReviewResult, Severity
...
def _compose_top_body(
    ...
    *,
    ...
    counts = _severity_counts(review.findings)
    non_zero = [f"{n} {sev}" for sev, n in counts.items() if n > 0]
    summary_tail = f" — {', '.join(non_zero)}" if non_zero else ""
    parts: list[str] = [
        f"Verdict: {review.verdict.value.upper()}{summary_tail}",
        "",
        (f"Generated by CodeSensei via {review.provider} in {review.elapsed_ms} ms."),
    ]
    if locationless:
        parts.extend(
