...
- `/review` — paste a GitHub PR URL → structured findings rendered grouped by file. Optionally pick an indexed repository to enable RAG-augmented review (the response then surfaces the files that contributed retrieved context). One-click "Post to GitHub" publishes the review back to the PR as a native review with inline comments via `POST /api/review/post` (uses the `codesensei-bot` PAT from `/settings`). See [`specs/003-pr-review-mvp/quickstart.md`](specs/003-pr-review-mvp/quickstart.md) for the original diff-only flow, [`specs/005-rag-indexing/quickstart.md`](specs/005-rag-indexing/quickstart.md) for the RAG flow, and [`specs/006-pr-review-posting/quickstart.md`](specs/006-pr-review-posting/quickstart.md) for the GitHub-posting flow.
