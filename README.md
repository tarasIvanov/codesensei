...
2. **Persistent AST-RAG index.** Whole repositories — not just diffs — are AST-chunked via tree-sitter using the cAST algorithm: greedy-merge sibling declarations up to a token budget, recursive split on oversize singletons, gap-emit imports / module-level constants. Multi-language support out of the box (Python, TypeScript, JavaScript, Go, Rust, Java, C, C++, Markdown, Ruby, PHP, Kotlin, Swift) via `tree-sitter-language-pack`; sliding-window stays as the fallback (ADR-017). Chunks embed into PostgreSQL + pgvector with HNSW for cosine similarity (ADR-004). Reviews retrieve overlap chunks + vector top-K context that gets included in the prompt.
