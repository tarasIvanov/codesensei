...
- `/history` — every successful review run is auto-saved (verdict + findings + provider + elapsed_ms + per-finding temporal context). The page lists the most-recent 50 runs newest-first with verdict filter chips; clicking a row reopens the run's detail view rendering the same findings with no fresh LLM call. From the detail view you can re-post a stored PR-URL run to GitHub, re-run the same input, or delete the row (CASCADE wipes its findings). Retention auto-prunes at 1000 rows. See [`specs/009-history-and-reports/quickstart.md`](specs/009-history-and-reports/quickstart.md) for the smoke walkthrough.
